...
namespace RoslynEditor.Core.Scripting
{
    ...
    public sealed partial class ScriptRunner
    {
        ...
        public ScriptRunner(string? code, ImmutableList<SyntaxTree>? syntaxTrees = null, CSharpParseOptions? parseOptions = null,
            OutputKind outputKind = OutputKind.DynamicallyLinkedLibrary, Platform platform = Platform.AnyCpu,
            IEnumerable<MetadataReference>? references = null, IEnumerable<string>? usings = null, string? filePath = null,
            string? workingDirectory = null, MetadataReferenceResolver? metadataResolver = null,
            SourceReferenceResolver? sourceResolver = null, InteractiveAssemblyLoader? assemblyLoader = null,
        ...
        public ImmutableArray<Diagnostic> Compile(Action<Stream>? peStreamAction, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                return ImmutableArray.CreateRange(GetCompilation(GetScriptAssemblyName()).GetDiagnostics(cancellationToken)
                    .Where(d => d.Severity == DiagnosticSeverity.Warning));
            }
            catch (CompilationErrorException e)
            {
                return ImmutableArray.CreateRange(e.Diagnostics
            ...
            DiagnosticBag diagnostics, CancellationToken cancellationToken)
        {
            ...
            if (_registerDependencies)
                foreach (var referencedAssembly in compilation.References
                    .Select(x => new { Key = x, Value = compilation.GetAssemblyOrModuleSymbol(x) as IAssemblySymbol }))
                {
                    if (referencedAssembly.Value == null)
                        continue;
        ...
        {
            var entryPointTypeName = BuildQualifiedName(entryPoint.ContainingNamespace.MetadataName,
        ...
        private static string BuildQualifiedName(string qualifier, string name) =>
            !string.IsNullOrEmpty(qualifier) ? string.Concat(qualifier, ".", name) : name;
        ...
        private Compilation GetCompilationFromCode(string assemblyName)
        {
            ...
            {
                if (Code == null)
                    throw new InvalidOperationException($"Either specify {nameof(Code)} or {nameof(SyntaxTrees)}");
            ...
            var compilationOptions = new CSharpCompilationOptions(OutputKind, mainTypeName: null, scriptClassName: "Program",
                usings: Usings, optimizationLevel: _optimizationLevel, checkOverflow: _checkOverflow, allowUnsafe: _allowUnsafe,
                platform: Platform, warningLevel: 4, deterministic: true, xmlReferenceResolver: null,
                sourceReferenceResolver: SourceResolver, metadataReferenceResolver: MetadataResolver,
                assemblyIdentityComparer: AssemblyIdentityComparer.Default, nullableContextOptions: NullableContextOptions.Enable
            );
        ...
        private static void ThrowIfAnyCompilationErrors(DiagnosticBag diagnostics, DiagnosticFormatter formatter)
        {
            ...
            if (!filtered.IsEmpty)
                throw new CompilationErrorException(formatter.Format(filtered[0], CultureInfo.CurrentCulture), filtered);
        }
        ...
        private static ImmutableArray<Diagnostic> GetDiagnostics(DiagnosticBag diagnostics, bool includeWarnings)
        {
            ...
            return diagnostics.AsEnumerable()
                ...
                (includeWarnings && d.Severity == DiagnosticSeverity.Warning)).AsImmutable();
        }
