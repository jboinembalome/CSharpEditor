...
namespace RoslynEditor.Core.Rename
{
    public static class RenameHelper
    {
            ...
            Document document, SyntaxToken triggerToken, CancellationToken cancellationToken)
        {
            ...
            if (symbol.IsImplicitlyDeclared && symbol.Kind != SymbolKind.Local && !(symbol.Kind == SymbolKind.Parameter &&
                  symbol.ContainingSymbol.Kind == SymbolKind.Method && symbol.ContainingType != null && 
                  symbol.ContainingType.IsDelegateType() && symbol.ContainingType.AssociatedSymbol != null))
                return null;
            ...
            foreach (var location in symbolLocations)
            {
                ...
                if (location.IsInSource)
                    if (document.Project.IsSubmission)
                    {
                        ...
                        if (solution.Projects.Any(p => p.IsSubmission && 
